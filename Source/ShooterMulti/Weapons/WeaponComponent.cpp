...
bool UWeaponComponent::Shot(FTransform cameraTransform)
{
...
bool UWeaponComponent::ShootLaser(AActor* Causer, FHitResult& HitResult, const FLaserWeaponData& WeaponData)
{
	...
	{
		if(HitResult.Actor->ActorHasTag("Cube"))
		{
			UStaticMeshComponent* mesh = Cast<UStaticMeshComponent>(HitResult.Actor->GetRootComponent());

			mesh->AddForce(LookDirection * 10000 * mesh->GetMass());
			return Cast<ACharacter>(HitResult.Actor) == nullptr;
		}
